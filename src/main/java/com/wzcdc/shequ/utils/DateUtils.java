...
public final class DateUtils {
    ...
    public static boolean checkTime(String time) {
        if (checkHHMM(time)) {
            ...
            if ((temp[0].length() == 2 || temp[0].length() == 1)
                    && temp[1].length() == 2) {
                int h, m;
                try {
    ...
    public static String topicTimeFomat(Date topicTime) throws ParseException {
        ...
        long diffBeforeEnd = endBefore.getTime().getTime()
                - topicTime.getTime();// 前天23:59:59与发帖时间之差
        long diffYearStart = startYear.getTime().getTime()
                - topicTime.getTime();// 当年1月1日
        // 00:00:00与发帖时间之差
     ...
     * 判断某一时间是否在一个区间内
     *
     * @param sourceTime 时间区间,半闭合,如[09:00-22:30)
     ...
     * @param beginTime     开始时间（24小时制）
     * @param appointMinute 预约时长（单位：分钟）
     * @return
     */
    public static boolean isAppointTimePass(Date appointDate, String beginTime,
                                            int appointMinute) {
        Calendar cal = Calendar.getInstance();
     ...
     * @return
     */
    public static Date getAppointStarDate(Date appointDate, String beginTime,
                                          int appointMinute) {
        Calendar cal = Calendar.getInstance();
        cal.setTime(appointDate);
        cal.set(Calendar.HOUR_OF_DAY, Integer.parseInt(beginTime.split(":")[0]));
     ...
     * @return
     */
    public static Date getAppointEndDate(Date appointDate, String beginTime,
                                         int appointMinute) {
        Calendar cal = Calendar.getInstance();
        cal.setTime(appointDate);
        cal.set(Calendar.HOUR_OF_DAY, Integer.parseInt(beginTime.split(":")[0]));
     ...
     * 将D8格式的时间字符串转化为日期对象
     *
     * @param d8String D8格式的时间字符串
     * @return 日期对象
     */
    public static Date StringtoDateD8(String d8String) {
        Date date = null;
        SimpleDateFormat sdf = new SimpleDateFormat("YYYYMMDD");
        try {
            date = sdf.parse(d8String);
        } catch (ParseException e) {
            return null;
        }
        return date;
    ...
    /**
     * 将DT15格式的时间字符串转化为日期对象
     *
     * @param dt15String DT15格式的时间字符串
     * @return 日期对象
     */
    public static Date StringtoDateDT15(String dt15String) {
        Date date = null;
        SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMdd'T'HHmmss");
        try {
            date = sdf.parse(dt15String);
        } catch (ParseException e) {
            return null;
        }
        return date;
    ...
    public static void main(String[] args) {
        ...
        System.out.println(DateUtils.StringtoDateDT15("20190617T174535"));
    }
